...
def get_jwt_token(access_token, open_id, platform, login_platform):
    ...
    try:
        ...
        return {
            'success': False,
            'error': 'Tất cả các URL đều không thành công',
            'message': 'Không thể lấy mã thông báo JWT từ bất kỳ máy chủ nào'
        }
...
async def get_token(access_token: str):
    ...
    if jwt_result['success']:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Việc trích xuất mã thông báo không thành công: {jwt_result.get('error', '')} - {jwt_result.get('message', '')}"
        )
